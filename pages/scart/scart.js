...
Page({
  ...
  onShow: function () {
    ...
    var that = this
    wx.request({
      url: 'https://jwshop.gesilaa6.club/index.php/index/caco/getAllFromCart.html',
  ...
  checkbox: function (e) {
    ...
    for (var i = 0; i < e.detail.value.length; i++) {
      ...
      wx.request({
        ...
        data: { openId: app.globalData.openId, commId: e.detail.value[i] },
        success: function (res) {
          //计算总价
  ...
  all: function (e) {
    ...
    for (var i = 0; i < this.data.scart.length; i++) {
      ...
      //计算总价
      sum += parseFloat(this.data.scart[i].price*this.data.scart[i].quantity)
      //根据点击次数修改复选框样式和总价
      if (cnumber % 2 == 0) {//全选
  ...
  delete: function () {
    ...
    for (var i = 0; i < this.data.array.length; i++) {
      wx.request({
        ...
        data: { openId: app.globalData.openId, type: 0, commId: this.data.array[i] },
        success: function () {
